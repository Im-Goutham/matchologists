...
import MediafileLoader from '../Loading/MediafileLoader';
...
import * as queactions from "../Questionnaire/question.action";

let gender_arr = ["MALE", "FEMALE"];
let interest_arr = ["MALE", "FEMALE", "BOTH"];
...
export const defaultMinDate = new Date().addHours(-6570 * 5);
export const defaultMaxDate = new Date().addHours(6570 * 5);
export const readableDate = date => date ? date.toISOString().substr(0, 19).replace('T', ' ') : 'undefined'

class Editprofile extends BaseFormComponent {
    constructor(props) {
        super(props);
        this.state = {
            date:"",
    ...
    Addreview() {
        console.log("OPEN")
        this.setState({ review_visible: true });
    }
    InviteFriend() {
        console.log("OPEN")
        this.setState({ friends_invite_visible: true });
    }
    generateInviteFriendToken() {
        let header = {
            'Authorization': this.props.token,
        }
        ApiManager.callwebservice('GET', 'api/generateInviteFriendToken', header, '', (success) => {
            let response = JSON.parse(success._bodyInit);
            console.log("generateInviteFriendToken", response)
            if (response.status === 0) {
                return
            }
            else if (response.status === 1) {
                reviewOptions.title = "React Native";
                reviewOptions.message = "";
                reviewOptions.url = response.data;
                reviewOptions.subject = "Share Link";
            }
        },
            (error) => {
    ...
    getUserprofile() {
        ...
        ApiManager.callwebservice('GET', 'api/profile', header, '', (success) => {
            let response = JSON.parse(success._bodyInit);
            ...
            if (response.status === 0) {
                ...
                let gender = response.data.gender ? response.data.gender.toUpperCase() : 0;
                let intersted_gender = response.data.genderIntrestedIn ? response.data.genderIntrestedIn.toUpperCase() : 0;
                let genderIndex = response.data.gender ? gender_arr.findIndex(k => k == gender) : 0;
                let interestedgenderIndex = response.data.genderIntrestedIn ? interest_arr.findIndex(k => k == intersted_gender) : 1;
                let date = new Date(response.data.dob);
                // var date_string = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate();
                var date_string = response.data.dob ? date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate() : "DD-MM-YYYY";
                getdob = response.data.dob ? date.getDate() + '-' + (date.getMonth() + 1) + '-' + date.getFullYear() : "DD-MM-YYYY";
                var is_dobSeleted = response.data.dob ? true : false;
    ...
    async componentDidMount() {
        await this.generateInviteFriendToken()
    ...
    _handleDatePicked = () => {
        ...
        let today = this.state.date, //new Date('Thu Dec 12 2018 19:08:04 GMT+0530 (IST)')
            currentdate = today.getFullYear() + '-' + parseInt(today.getMonth() + 1) + '-' + today.getDate(),
            getdobdisplay = today.getDate() + '-' + parseInt(today.getMonth() + 1) + '-' + today.getFullYear();
            console.log("currentdate", this.state.date)

        if (todayDate.getFullYear() - today.getFullYear() <= 16) {
            this.showSimpleMessage("info", { backgroundColor: global.gradientsecondry }, I18n.t('validation.dob_notallowlable', { locale: language }), I18n.t('validation.dob_notallowtitle', { locale: language }))
            return false
        } else {
            this.setState({
                is_dobSeleted: true,
                getdob: getdobdisplay,
                dob: currentdate //date.toString() 
            }, () => { 
    ...
    upload_pic = (filelink, filetype, filename, is_video, order) => {
        return new Promise(async (resolve, reject) => {
            // const { thumblinefiletype, thumblinename } = this.state;
            this.setState({ is_uploadFile: true })
            let header = {
                'Authorization': this.props.token,
            };
            var details = {
            ...
            ApiManager.callRnFetchblobFileUploader('POST', 'api/uploadFile', header, details, (progress) => {
                this.updateUploadingProgress(progress)
            }, (success) => {
                ...
                if (response.status === 0) {
                    ...
                    var arr = this.state.gallary_Images;

                    console.log("response_upload_pic", response)
                    console.log("upload_pic_order", order)
                    let fileImage = response.data && response.data.filepath ? response.data.filepath : undefined;
                    console.log("fileImage", fileImage)
                    var index = _.findIndex(arr, { order: order });
                    ...
                    if (index === -1) {
                        ...
                        arr[index].url = fileImage;
                        arr[index].isVideo = is_video;
                        arr[index].isUpdated = Is_update;
                    }
                    this.setState({
                        gallary_Images: arr,
                        is_uploadFile: false
    ...
    bioValidation(){
        ...
        if (!bio.length) {
            this.showSimpleMessage("info", { backgroundColor: global.gradientsecondry }, I18n.t('validation.biolabel', { locale: language }), I18n.t('validation.biotitle', { locale: language }))
            return false;
        }
        if (bio.length < 132) {
            this.showSimpleMessage("info", { backgroundColor: global.gradientsecondry }, I18n.t('validation.bioshortlabel', { locale: language }), I18n.t('validation.bioshorttitle', { locale: language }))
            return false;
        }
        return true;
    }
    ...
    validate() {
        const { language } = this.props;
        const { fullname, selectedgender, interest_gender, is_dobSeleted, bio, gallary_Images } = this.state;
        if (!fullname.length) {
            this.showSimpleMessage("info", { backgroundColor: global.gradientsecondry }, I18n.t('validation.fullname_label', { locale: language }), I18n.t('validation.fullname_title', { locale: language }))
            return false;
        }
        ...
        if (!gallary_Images.length) {
            this.showSimpleMessage("info", { backgroundColor: global.gradientsecondry }, I18n.t('validation.imageLabel', { locale: language }), I18n.t('validation.imagetitle', { locale: language }))
            return false;
        }
        return true
    }
    ...
    updateProfile = async (uploadimage) => {
        ...
        details = {
            "fullName": fullname,
            "gender": gender_arr[selectedgender].toLowerCase(),
        ...
        if (this.validate()) {
            this.setState({ is_Loading: !this.state.is_Loading })
            await this.asyncForEach(gallary_Images, async (image, index) => {
                if (image.is_Update) {
                    let path = image.uri;
                    let filetype = image.filetype;
                    let name = image.filename;
                    let is_video = image.is_video;
                    let order = image.order;
                    await this.upload_pic(path, filetype, name, is_video, order)
                }
            });
            // for (let i = 0; i < gallary_Images.length; i++) {
            //     console.log("gallary_Images", gallary_Images)
            //     if (gallary_Images[i].is_Update) {
            //         let path = gallary_Images[i].uri;
            //         let filetype = gallary_Images[i].filetype;
            //         let name = gallary_Images[i].filename;

            //         let is_video = gallary_Images[i].is_video;
            //         let order = gallary_Images[i].order;
            //         this.upload_pic(path, filetype, name, is_video, order)
            //     }
            // }
            // console.log("validate_details", details)
            // alert("hello apicall")
            // console.log("validate_details again", details)
            details.gallery = this.state['gallary_Images'];
            await ApiManager.callwebservice('POST', 'api/updateUserProfile', header, details, (success) => {
                let response = JSON.parse(success._bodyInit);
                if (response.status === 0) {
                    this.showSimpleMessage("danger", { backgroundColor: "#DC6666" }, response.message, '')
                    this.setState({ is_Loading: !this.state.is_Loading })
    ...
    updateUploadingProgress = (loadingProgress) => {
        console.log("loadingProgress", loadingProgress)
        this.setState({ loadingProgress })
    }
    gallary_Images_update(cbdata) {
        console.log("cbdata", cbdata);
        let Localdata = this.state.gallary_Images;
        console.log("cbdata", Localdata.length);
        var index = _.findIndex(Localdata, { order: cbdata.order });
        console.log("findIndex", index);
        if (index < 1 && index === -1) {
            Localdata.push(cbdata)
        } else {
            Localdata.splice(index, 1, cbdata);
        }
        this.setState({
            gallary_Images: Localdata
        })
    }
    render() {
        const { isDateTimePickerVisible, dob, selectedgender, interest_gender, is_Loading, fullname, bio, gallary_Images, is_uploadFile, getdob } = this.state;
        const { navigate } = this.props.navigation;
            ...
            <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
                ...
                <Header
                            ...
                            middle={
                                <View style={{ width: "70%", backgroundColor: "transparent", alignItems: "center", justifyContent: "center" }}>
                                    <Text style={{ fontFamily: "Avenir-Heavy", fontSize: 24, color: "#fff" }}>Edit Profile</Text>
                                </View>
                            }
                            right={
                                <View style={{
                ...
                <ScrollView style={styles.container}>
                    <View style={styles.questionBox}>
                        <Text style={styles.questionTitle}>{I18n.t("aboutPageLabel", { locale: language })}</Text>
                    </View>
                                ...
                                this.state.pageIndex === 1
                                    ? <>
                                        <View style={styles.formgroup}>
                                            <Text style={styles.label}>{I18n.t('namelabel', { locale: language })}</Text>
                                            <CustomTextInput
                                                name={'name'}
                                                ref={(ref) => this.nameInputRef = ref}
                                                value={this.state.fullname}
                                                // placeholder={"Firstname  Middle name Last name"}
                                        ...
                                        <View style={styles.formgroup}>
                                            <Text style={styles.label}>{I18n.t('dob_label', { locale: language })}</Text>
                                        ...
                                        <View style={styles.formgroup}>
                                            <Text style={styles.label}>{I18n.t('gender_label', { locale: language })}</Text>
                                            <View style={{
                                                height: 42,
                                                borderWidth: 1,
                                                borderColor: "#F5F5F5",
                                        ...
                                        <View style={styles.formgroup}>
                                            <Text style={styles.label}>{I18n.t('interest_label', { locale: language })}</Text>
                                            <View style={{
                                                height: 42,
                                                borderWidth: 1,
                                                borderColor: "#F5F5F5",
                                                    ...
                                                    interest_arr.map((value, key) => {
                                                        return (
                                                            interest_gender === key ?
                                                                ...
                                                                :
                                                                <TouchableOpacity key={key} onPress={() => this.setState({ interest_gender: key })}
                                                                    style={{ flex: 1, alignItems: "center", justifyContent: "center", backgroundColor: "#FFF", borderRadius: 21 }}>
                                                                    <Text style={{
                                                                        fontFamily: "Avenir-Heavy",
                                                                        fontSize: 17,
                                    ...
                                    this.state.pageIndex === 2
                                        ...
                                        <View style={styles.formgroup}>
                                            <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                                                <Text style={styles.label}>{I18n.t('bio_label', { locale: language })}</Text>
                                                <Text style={styles.label}>{'Min 132 Character'}</Text>
                                            ...
                                            <View style={{
                                                ...
                                                <CustomTextInput
                                                    ...
                                                    placeholder={I18n.t('bioPlaceholder', { locale: language })}
                                                    numberOfLines={4}
                                                    multiline={true}
                                                    name={'bio'}
                                        ...
                                        this.state.pageIndex === 3 ?
                                            <View style={[styles.formgroup]}>
                                                <Text style={styles.label}>{I18n.t('gallary_label', { locale: language })}</Text>
                                                <Gallary
                                                    token={this.props.token}
                                                    gallary_Images={this.state.gallary_Images}
                                                    refreshpage={() => this.getUserprofile()}
                                                    // fullname={fullname}
                                                    // dob={dob}
                                                    ...
                                                    gallary_Images_update={this.gallary_Images_update.bind(this)}
                                                // updateUploadingProgress={this.updateUploadingProgress.bind(this)}
                                                />
                                            </View>
                                            : undefined
                            }
                            {/* <TouchableOpacity style={[styles.inviteFriend]} onPress={() => this.Addreview()}>
                                <Text style={styles.invitefirend_button_text}>{I18n.t('addreviewtitle', { locale: language })}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style={styles.inviteFriend} onPress={() => this.InviteFriend()}>
                                <Text style={styles.invitefirend_button_text}>{I18n.t('invitefriendtitle', { locale: language })}</Text>
                            </TouchableOpacity> */}
                            {
                                this.state.pageIndex === 1 ?
                                    <>
                                        ...
                                        <LinearGradient
                                            ...
                                            <CustomButton
                                                text={"NEXT"}
                                                // text={I18n.t('start_button', { locale: language })}
                                                // onPress={() => navigate('questionnaire')}
                                                onPress={() => {this.validateFirstScreen() ? this.setState({ pageIndex: 2 }): undefined }}
                                        ...
                                        <>
                                            ...
                                            <View style={{ flexDirection: "row", flex: 1, justifyContent: "space-between" }}>
                                                <LinearGradient
                                                    ...
                                                    style={[styles.signInButton, { width: '45%' }]}>
                                                    <CustomButton
                                                        text={"BACK"}
                                                        // onPress={() => navigate('questionnaire')}
                                                        onPress={() => this.setState({ pageIndex: 1 })}
                                                        textStyle={styles.signInButtonText}
                                                    />
                                                </LinearGradient>
                                                <LinearGradient
                                                    ...
                                                    <CustomButton
                                                        text={"NEXT"}
                                                        // onPress={() => navigate('questionnaire')}
                                                        onPress={() => {this.bioValidation()? this.setState({ pageIndex: 3 }): undefined}}
                                                        textStyle={styles.signInButtonText}
                                                    />
                                                </LinearGradient>
                                            </View>
                                                ...
                                                <View style={{ flexDirection: "row", flex: 1, justifyContent: "space-between" }}>
                                                    <LinearGradient
                                                        ...
                                                        <CustomButton
                                                            text={"BACK"}
                                                            // onPress={() => navigate('questionnaire')}
                                                            onPress={() => this.setState({ pageIndex: 2 })}
                                                            textStyle={styles.signInButtonText}
                                                        />
                                                    </LinearGradient>
                                                    <LinearGradient
                ...
                <Modal
                    ...
                    style={{ margin: 0, padding: 0, justifyContent: "flex-end", alignItems: "center" }}>
                    <View style={{ backgroundColor: "#FFF", borderRadius:5, overflow:"hidden" }}>
                        <DatePicker
                        ...
                        />
                        <TouchableOpacity onPress={()=>this._handleDatePicked()} style={{ height:54, justifyContent:"center", alignItems:"center", borderColor:"#909096"  ,borderWidth:StyleSheet.hairlineWidth}}>
                            <Text style={{ fontFamily: "Avenir-Heavy", fontSize: 17,color: "#DB3D88"}}>Confirm</Text>
                            </TouchableOpacity>
                ...
                /> */}
                {/* share  */}
                {/* <ShareSheet visible={this.state.review_visible} onCancel={() => this.onCancel()}>
                    <Button iconSrc={{ uri: EMAIL_ICON }}
                        onPress={() => {
                            this.onCancel();
                            setTimeout(() => {
                                Share.shareSingle(Object.assign(reviewOptions, {
                                    "social": "email"
                                }));
                            }, 300);
                        }}>Email</Button>
                </ShareSheet>
                <ShareSheet visible={this.state.friends_invite_visible} onCancel={() => this.onCancel()}>
                ...
                <Modal
                    isVisible={is_uploadFile}
                    scrollTo={this.handleScrollTo}
                    style={{ margin: 0, justifyContent: "center", alignItems: "center", flexDirection: "column" }}>
                    <Text style={{ color: "#fff", fontSize: 15 }}> {this.state.loadingProgress} %</Text>
                    <MediafileLoader />
                </Modal>
                {/* share close */}
            </SafeAreaView>
...
class Gallary extends BaseFormComponent {
    constructor(props) {
        super(props);
        this.state = {
    ...
    componentDidMount() {
        // console.log("componentDidMount",this.props.gallary_Images)
        var i = 0;
        while (i < 6) {
            var key = "image_" + i
            var val = {
            ...
            var obj = {}
            obj[key] = val
            console.log("all stated data", obj)
            this.setState(obj)
            i++;
        }
        let data = this.props.gallary_Images;
        var data_arr = [];
        if (this.props.gallary_Images && this.props.gallary_Images.length) {
            var data_arr = [];
            for (var i = 0; i < data.length; i++) {
                var arr = {};
                arr.order = data[i].order;
                arr._id = data[i]._id;
                if (data[i].isVideo) {
                    arr.uri = data[i].thumbImage;
                    arr.isVideo = data[i].isVideo;
                } else {
                    arr.uri = data[i].uri ? data[i].uri : data[i].url;
                }
                data_arr.push(arr)
            }
            var i = 0;
    ...
    componentWillReceiveProps = (nextProps, nextState) => {
        let data = nextProps.gallary_Images;
        console.log("componentWillReceiveProps", data)
        this.setState({
            // fullname: nextProps.fullname,
            // dob: nextProps.dob,
            gallary_Images: nextProps.gallary_Images
        })
    ...
    selectPhotoTapped() {
        const { fullname, dob, gallary_Images, order_number } = this.state;
        ...
        ImagePicker.showImagePicker(options, (response) => {
            ...
            if (response.didCancel) {
                ...
                console.log("selectPhotoTapped Path:=", response)
                let filetype = 'image/jpg';
                let is_video = false;
                setLocal.uri = path;
                setLocal.filetype = filetype;
                setLocal.is_video = is_video;
                setLocal.order = order_number.toString();
                setLocal.is_Update = true;
                setLocal.filename = name;
                this.setState({
                    thumbnail_image: source,
                    thumblinefiletype: response.type,
                    imageSelect: true,
                    videoSelect: false,
                    thumblinename: name,
                }, this.props.gallary_Images_update(setLocal))
            }
        });
    }
    selectVideoTapped() {
    ...
    VideoGraphics() {
        return (
            <View style={{ position: "absolute", zIndex: 3, height: 17, width: 17, top: 7, left: 7, borderRadius: 8 }}>
                <Image source={require('../../images/icons/Videos.png')} style={{ height: '100%', width: '100%' }} />
            </View>
        )
    }
    render() {
        return (
            <View style={[styles.elevated_shdow, { flex: 1, marginTop: 8 }]}>
                <View style={{ flexDirection: "row" }}>
                    <TouchableOpacity style={[styles.elevated_shdow, { flex: 6.5, borderRadius: 10, overflow: "hidden" }]} onPress={() => this.showActionSheet(this.state.image_0.order)} >
                        {
                            this.state.image_0.isVideo ? this.VideoGraphics() : undefined
                        }
                        <Image
                            source={!this.state.image_0.uri ? require('../../images/applogo.png') : this.state.image_0}
                        ...
                        // resizeMode="contain"
                        />
                        <Text style={[styles.elevated_shdow, styles.image_badge]}>{this.state.image_0.order}</Text>
                    </TouchableOpacity>
                    <View style={{ justifyContent: "space-between", alignItems: 'flex-end', flex: 3.5, }}>
                        <TouchableOpacity style={[styles.elevated_shdow, {
                            borderRadius: 5,
                            // marginVertical: 5,
                            ...
                            onPress={() => this.showActionSheet(this.state.image_1.order)}>
                            {this.state.image_1.isVideo ? this.VideoGraphics() : undefined}
                            <Image
                                source={!this.state.image_1.uri ? require('../../images/applogo.png') : this.state.image_1}
                                style={{
                                    flex: 1,
                                    width: null,
                                    height: null,
                                ...
                                resizeMethod="resize"
                            />
                            <Text style={[styles.elevated_shdow, styles.image_badge]}>{this.state.image_1.order}</Text>
                        </TouchableOpacity>
                        <View style={{ height: 5 }} />
                        <TouchableOpacity
                            style={[styles.elevated_shdow, {
                                borderRadius: 5,
                                // marginVertical: 5,
                                width: metrics.DEVICE_WIDTH * 0.25,
                                height: metrics.DEVICE_WIDTH * 0.25,
                            ...
                            onPress={() => this.showActionSheet(this.state.image_2.order)}>
                            {this.state.image_2.isVideo ? this.VideoGraphics() : undefined}
                            <Image
                                source={!this.state.image_2.uri ? require('../../images/applogo.png') : this.state.image_2}
                                style={{
                                    flex: 1,
                                    width: null,
                                    height: null,
                                ...
                                resizeMode="contain"
                            />
                            <Text style={[styles.elevated_shdow, styles.image_badge]}>{this.state.image_2.order}</Text>
                        </TouchableOpacity>
                    </View>
                </View>
                <View style={{ height: metrics.DEVICE_WIDTH * 0.28, flexDirection: "row", justifyContent: "space-between", alignItems: 'flex-end' }}>
                    <TouchableOpacity style={[styles.elevated_shdow, {
                        borderRadius: 5, width: metrics.DEVICE_WIDTH * 0.25,
                        height: metrics.DEVICE_WIDTH * 0.25,
                        backgroundColor: "transparent",
                        overflow: "hidden"
                    }]}
                        onPress={() => this.showActionSheet(this.state.image_3.order)}>
                        {this.state.image_3.isVideo ? this.VideoGraphics() : undefined}
                        <Image
                            source={!this.state.image_3.uri ? require('../../images/applogo.png') : this.state.image_3}
                            style={{
                                flex: 1,
                                width: null,
                                height: null,
                        ...
                        // resizeMode="contain"
                        />
                        <Text style={[styles.elevated_shdow, styles.image_badge]}>{this.state.image_3.order}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.elevated_shdow, {
                        borderRadius: 5, width: metrics.DEVICE_WIDTH * 0.25,
                        height: metrics.DEVICE_WIDTH * 0.25,
                        backgroundColor: "transparent",
                        overflow: "hidden"
                    }]} onPress={() => this.showActionSheet(this.state.image_4.order)}>
                        {this.state.image_4.isVideo ? this.VideoGraphics() : undefined}
                        <Image
                            source={!this.state.image_4.uri ? require('../../images/applogo.png') : this.state.image_4}
                            style={{
                                flex: 1,
                                width: null,
                                height: null,
                        ...
                        // resizeMode="contain"
                        />
                        <Text style={[styles.elevated_shdow, styles.image_badge]}>{this.state.image_4.order}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.elevated_shdow, {
                        borderRadius: 5, width: metrics.DEVICE_WIDTH * 0.25,
                        height: metrics.DEVICE_WIDTH * 0.25,
                        ...
                        onPress={() => this.showActionSheet(this.state.image_5.order)}>
                        {this.state.image_5.isVideo ? this.VideoGraphics() : undefined}

                        <Image
                            source={!this.state.image_5.uri ? require('../../images/applogo.png') : this.state.image_5}
                            style={{
                                flex: 1,
                                width: null,
                                height: null,
                            ...
                            resizeMethod="resize"
                        />
                        <Text style={[styles.elevated_shdow, styles.image_badge]}>{this.state.image_5.order}</Text>
                    </TouchableOpacity>
                </View>
                <ActionSheet
                    ref={o => this.ActionSheet = o}
                    title={'Which File Do You like to Upload ? '}
                    options={['Image', 'Video', 'Cancel']}
                    cancelButtonIndex={2}
                    destructiveButtonIndex={1}
                    onPress={(index) => { this.selectFile(index) }}
                />
            </View>
        )
    }
...
mapDispatchToProps = (dispatch) => {
    return bindActionCreators({
        saveprofileStatus: actions.visitprofile,
        saveNumber: queactions.savegivenanswerNumber
    }, dispatch);
}

import React, { Component, PropTypes } from 'react'
import {
    KeyboardAvoidingView,
    ToastAndroid,
    LayoutAnimation,
    AlertIOS,
    Platform,
    StyleSheet,
    StatusBar,
    SafeAreaView,
    UIManager,
    Text,
    ScrollView,
    View,
    Image,
    TouchableOpacity,
} from 'react-native'
import { connect } from 'react-redux'
import { bindActionCreators } from "redux";
import _ from 'lodash';
import LinearGradient from 'react-native-linear-gradient';
import I18n from 'react-native-i18n';
import DateTimePicker from 'react-native-modal-datetime-picker';
import CustomButton from '../CustomButton'
import metrics from '../../config/metrics'
import CustomTextInput from '../CustomTextInput';
import BaseFormComponent from "../Common/BaseFormComponent";
import ImagePicker from 'react-native-image-picker'
import ActionSheet from '../Common/ActionSheet/index'
import Share, { ShareSheet, Button } from 'react-native-share';
import Loader from '../Loading';
import MediafileLoader from '../Loading/MediafileLoader';
import Modal from "react-native-modal";
import ApiManager from "../Common/ApiManager";
import * as global from '../../../global.json'
import * as actions from "../AuthScreen/auth.actions";
import * as queactions from "../Questionnaire/question.action";

let gender_arr = ["MALE", "FEMALE", "OTHER"];
let interest_arr = ["MALE", "FEMALE", "BOTH"];
const IS_ANDROID = Platform.OS === 'android'
if (IS_ANDROID) UIManager.setLayoutAnimationEnabledExperimental(true)
const formStyle = { marginTop: 40 };

let reviewOptions = {
    title: "",
    message: "",
    url: "",
    subject: ""
};
let shareOptions = {
    title: "React Native",
    message: "Hola mundo",
    url: "https://itunes.apple.com/us/app/zero-2-two-app/id1374303629?ls=1&mt=8",
    subject: "Share Link"
};

class Basicinfo extends BaseFormComponent {
    constructor(props) {
        super(props);
        this.state = {
            fullname: '',
            selectedgender: 0,
            interest_gender: 0,
            isDateTimePickerVisible: false,
            is_dobSeleted: false,
            getdob: '',
            dob: "Select Date",
            bio: "",
            gallary_Images: [],
            is_Loading: false,
            review_visible: false,
            friends_invite_visible: false,
            is_uploadFile: false,
            loadingProgress: 1
        }
    }
    onCancel() {
        console.log("CANCEL")
        this.setState({
            review_visible: false,
            friends_invite_visible: false
        });
    }
    Addreview() {
        console.log("OPEN")
        this.setState({ review_visible: true });
    }
    InviteFriend() {
        console.log("OPEN")
        this.setState({ friends_invite_visible: true });
    }
    generateInviteFriendToken() {
        let header = {
            'Authorization': this.props.token,
        }
        ApiManager.callwebservice('GET', 'api/generateInviteFriendToken', header, '', (success) => {
            let response = JSON.parse(success._bodyInit);
            console.log("generateInviteFriendToken", response)
            if (response.status === 0) {
                return
            }
            else if (response.status === 1) {
                reviewOptions.title = "React Native";
                reviewOptions.message = "Hola mundo";
                reviewOptions.url = response.data;
                reviewOptions.subject = "Share Link";
            }
        },
            (error) => {
                console.log("error", error)
            })
    }
    getUserprofile() {
        let header = {
            'Authorization': this.props.token,
        }
        ApiManager.callwebservice('GET', 'api/profile', header, '', (success) => {
            let response = JSON.parse(success._bodyInit);
            console.log("responseprofile", response)
            if (response.status === 0) {
                this.showSimpleMessage("danger", { backgroundColor: "#DC6666" }, response.message, '')
                return
            }
            else if (response.status === 1) {
                let gender = response.data.gender ? response.data.gender.toUpperCase() : 0;
                let intersted_gender = response.data.genderIntrestedIn ? response.data.genderIntrestedIn.toUpperCase() : 0;
                let genderIndex = response.data.gender ? gender_arr.findIndex(k => k == gender) : 0;
                let interestedgenderIndex = response.data.genderIntrestedIn ? interest_arr.findIndex(k => k == intersted_gender) : 1;
                let date = new Date(response.data.dob);
                // var date_string = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate();
                var date_string = response.data.dob ? date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate() : "DD-MM-YYYY";
                getdob = response.data.dob ? date.getDate() + '-' + (date.getMonth() + 1) + '-' + date.getFullYear() : "DD-MM-YYYY";
                var is_dobSeleted = response.data.dob ? true : false;
                let galaryData = response.data.gallery;
                galaryData.sort(function (a, b) {
                    return a.order - b.order
                })
                this.setState({
                    fullname: response.data.fullName,
                    selectedgender: genderIndex,
                    interest_gender: interestedgenderIndex,
                    dob: date_string,
                    bio: response.data.bio ? response.data.bio : '',
                    gallary_Images: galaryData,
                    is_dobSeleted,
                    getdob
                })
            }
        }, (error) => {
            console.log("error", error)
        })
    }
    async componentDidMount() {
        await this.generateInviteFriendToken()
        await this.getUserprofile()
    }
    _showDateTimePicker = () => this.setState({ isDateTimePickerVisible: true });
    _hideDateTimePicker = () => this.setState({ isDateTimePickerVisible: false });
    _handleDatePicked = date => {
        const { language } = this.props;
        let todayDate = new Date();
        let today = date, //new Date('Thu Dec 12 2018 19:08:04 GMT+0530 (IST)')
            currentdate = today.getFullYear() + '-' + parseInt(today.getMonth() + 1) + '-' + today.getDate(),
            getdobdisplay = today.getDate() + '-' + parseInt(today.getMonth() + 1) + '-' + today.getFullYear();
        if (todayDate.getFullYear() - today.getFullYear() <= 16) {
            this.showSimpleMessage("info", { backgroundColor: global.gradientsecondry }, I18n.t('validation.dob_notallowlable', { locale: language }), I18n.t('validation.dob_notallowtitle', { locale: language }))
            this._hideDateTimePicker();
            return false
        } else {
            this.setState({
                is_dobSeleted: true,
                getdob: getdobdisplay,
                dob: currentdate //date.toString() 
            }, () => this._hideDateTimePicker());
        }
    };


    upload_pic = (callback) => {

        const { fullname, selectedgender, interest_gender, dob, bio, gallary_Images } = this.state;
        for (let i = 0; i < gallary_Images.length; i++) {
            console.log("gallary_Images", gallary_Images)
            if (gallary_Images[i].is_Update) {
                let path = gallary_Images[i].uri;
                let filetype = gallary_Images[i].filetype;
                let name = gallary_Images[i].filename;

                let is_video = gallary_Images[i].is_video;
                let order = gallary_Images[i].order;
                // const { thumblinefiletype, thumblinename } = this.state;
                this.activateLoader()
                // this.setState({ is_Loading: !this.state.is_Loading })
                let header = {
                    'Authorization': this.props.token,
                };
                var details = {
                    url: path,
                    type: filetype,
                    name: name
                };
                let Is_update = true
                console.log("uploadFile", details)

                ApiManager.callRnFetchblobFileUploader('POST', 'api/uploadFile', header, details, (progress) => {
                    this.updateUploadingProgress(progress)
                }, (success) => {
                    var response = JSON.parse(success.data);
                    console.log("fileUpload", response)
                    // this.setState({ is_Loading: !this.state.is_Loading })
                    if (response.status === 0) {
                        this.showSimpleMessage("danger", { backgroundColor: "#DC6666" }, response.message, '')
                        return
                    }
                    else if (response.status === 1) {
                        var arr = this.state.gallary_Images;

                        console.log("response_upload_pic", response)
                        console.log("upload_pic_order", order)
                        let fileImage = response.data && response.data.filepath ? response.data.filepath : undefined;
                        console.log("fileImage", fileImage)
                        var index = _.findIndex(arr, { order: order });

                        if (index === -1) {
                            arr.push({
                                order: order,
                                url: fileImage,
                                isVideo: is_video,
                                isUpdated: Is_update
                            })
                        } else {
                            arr[index].url = fileImage;
                            arr[index].isVideo = is_video;
                            arr[index].isUpdated = Is_update;
                            // arr.splice(index, 1, 
                            //     { 
                            //         order: order, 
                            //         url: fileImage,
                            //         is_video:is_video,
                            //         is_Update: Is_update
                            //     });
                        }
                        this.activateLoader()
                        console.log("response_upload_pic_index", arr)
                        // console.log("fileImage", arr)
                        this.setState({
                            gallary_Images: arr,
                            // is_Loading: !this.state.is_Loading
                        })
                        console.log('i == gallary_Images.length-1', i == gallary_Images.length-1);
                        if( i == gallary_Images.length-1){
                            callback();
                        }
                    }
                }, (error) => {
                    console.log("error", error)
                })
            }else{
                if( i == gallary_Images.length-1){
                    callback();
                }
            }
        }


    }
    // upload_pic = (filelink, filetype, filename, is_video, order, callback) => {
    //     // const { thumblinefiletype, thumblinename } = this.state;
    //     this.activateLoader()
    //     // this.setState({ is_Loading: !this.state.is_Loading })
    //     let header = {
    //         'Authorization': this.props.token,
    //     };
    //     var details = {
    //         url: filelink,
    //         type: filetype,
    //         name: filename
    //     };
    //     let Is_update = true
    //     console.log("uploadFile", details)

    //     ApiManager.callRnFetchblobFileUploader('POST', 'api/uploadFile', header, details, (progress) => {
    //         this.updateUploadingProgress(progress)
    //     }, (success) => {
    //         var response = JSON.parse(success.data);
    //         console.log("fileUpload", response)
    //         // this.setState({ is_Loading: !this.state.is_Loading })
    //         if (response.status === 0) {
    //             this.showSimpleMessage("danger", { backgroundColor: "#DC6666" }, response.message, '')
    //             return
    //         }
    //         else if (response.status === 1) {
    //             var arr = this.state.gallary_Images;

    //             console.log("response_upload_pic", response)
    //             console.log("upload_pic_order", order)
    //             let fileImage = response.data && response.data.filepath ? response.data.filepath : undefined;
    //             console.log("fileImage", fileImage)
    //             var index = _.findIndex(arr, { order: order });

    //             if (index === -1) {
    //                 arr.push({
    //                     order: order,
    //                     url: fileImage,
    //                     isVideo: is_video,
    //                     isUpdated: Is_update
    //                 })
    //             } else {
    //                 arr[index].url = fileImage;
    //                 arr[index].isVideo = is_video;
    //                 arr[index].isUpdated = Is_update;
    //                 // arr.splice(index, 1, 
    //                 //     { 
    //                 //         order: order, 
    //                 //         url: fileImage,
    //                 //         is_video:is_video,
    //                 //         is_Update: Is_update
    //                 //     });
    //             }
    //             this.activateLoader()
    //             console.log("response_upload_pic_index", arr)
    //             // console.log("fileImage", arr)
    //             this.setState({
    //                 gallary_Images: arr,
    //                 // is_Loading: !this.state.is_Loading
    //             })
    //         }
    //     }, (error) => {
    //         console.log("error", error)
    //     })
    // }

    validate() {
        const { language } = this.props;
        const { fullname, selectedgender, interest_gender, is_dobSeleted, bio, gallary_Images } = this.state;
        if (!fullname.length) {
            this.showSimpleMessage("info", { backgroundColor: global.gradientsecondry }, I18n.t('validation.fullname_label', { locale: language }), I18n.t('validation.fullname_title', { locale: language }))
            return false;
        }
        if (!is_dobSeleted) {
            this.showSimpleMessage("info", { backgroundColor: global.gradientsecondry }, I18n.t('validation.doblabel', { locale: language }), I18n.t('validation.dobtitle', { locale: language }))
            return false;
        }
        if (!bio.length) {
            this.showSimpleMessage("info", { backgroundColor: global.gradientsecondry }, I18n.t('validation.biolabel', { locale: language }), I18n.t('validation.biotitle', { locale: language }))
            return false;
        }
        if (bio.length < 50) {
            this.showSimpleMessage("info", { backgroundColor: global.gradientsecondry }, I18n.t('validation.bioshortlabel', { locale: language }), I18n.t('validation.bioshorttitle', { locale: language }))
            return false;
        }
        if (!gallary_Images.length) {
            this.showSimpleMessage("info", { backgroundColor: global.gradientsecondry }, I18n.t('validation.imageLabel', { locale: language }), I18n.t('validation.imagetitle', { locale: language }))
            return false;
        }
        return true
    }
    updateProfile = (uploadimage) => {
        const { navigate } = this.props.navigation;
        
        let header = {
            'Authorization': this.props.token,
            'Content-Type': 'application/json'
        }
       
        if (this.validate()) {
            this.setState({ is_Loading: !this.state.is_Loading })
            this.upload_pic( () => {
                console.log('in callback');
                
                const { fullname, selectedgender, interest_gender, dob, bio, gallary_Images } = this.state;
                
                details = {
                    "fullName": fullname,
                    "gender": gender_arr[selectedgender].toLowerCase(),
                    "genderIntrestedIn": gender_arr[interest_gender].toLowerCase(),
                    "dob": dob,
                    "bio": bio,
                    "gallery": gallary_Images
                }
                console.log("validate_details", details)
                console.log("validate_details again", details)
                ApiManager.callwebservice('POST', 'api/updateUserProfile', header, details, (success) => {
                    let response = JSON.parse(success._bodyInit);
                    if (response.status === 0) {
                        this.showSimpleMessage("danger", { backgroundColor: "#DC6666" }, response.message, '')
                        this.setState({ is_Loading: !this.state.is_Loading })
                        return
                    }
                    else if (response.status === 1) {
                        console.log("response", response)
                        this.getUserprofile()
                        this.props.saveprofileStatus()
                        navigate('questionnaire')
                        this.props.saveNumber(1)
                        this.setState({ is_Loading: !this.state.is_Loading })
                    }
                }, (error) => {
                    console.log("error", error)
                })
            })
            // for (let i = 0; i < gallary_Images.length; i++) {
            //     console.log("gallary_Images", gallary_Images)
            //     if (gallary_Images[i].is_Update) {
            //         let path = gallary_Images[i].uri;
            //         let filetype = gallary_Images[i].filetype;
            //         let name = gallary_Images[i].filename;

            //         let is_video = gallary_Images[i].is_video;
            //         let order = gallary_Images[i].order;
            //         this.upload_pic(path, filetype, name, is_video, order, ()=>{
            //             console.log('in callback');
            //         })
            //     }
            // }
           
        }

    }
    activateLoader() {
        this.setState({
            is_uploadFile: !this.state.is_uploadFile
        })
    }
    updateUploadingProgress = (loadingProgress) => {
        console.log("loadingProgress", loadingProgress)
        this.setState({ loadingProgress })
    }
    gallary_Images_update(cbdata) {
        console.log("cbdata", cbdata);
        let Localdata = this.state.gallary_Images;
        console.log("cbdata", Localdata.length);
        var index = _.findIndex(Localdata, { order: cbdata.order });
        console.log("findIndex", index);
        if (index < 1 && index === -1) {
            Localdata.push(cbdata)
        } else {
            Localdata.splice(index, 1, cbdata);
        }
        this.setState({
            gallary_Images: Localdata
        })
    }
    render() {
        const { isDateTimePickerVisible, dob, selectedgender, interest_gender, is_Loading, fullname, bio, gallary_Images, is_uploadFile, getdob } = this.state;
        const { navigate } = this.props.navigation;
        const { isLoading, onLoginLinkPress, onSignupPress, language } = this.props;
        // console.log("gallary_Images", gallary_Images)
        if (is_Loading) {
            return <Loader />
        }
        return (
            <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
                {Platform.OS === 'android' ? <StatusBar barStyle="dark-content" backgroundColor="#fff" /> : undefined}
                <ScrollView style={styles.container}>
                    <View style={styles.questionBox}>
                        <Text style={styles.questionTitle}>{I18n.t("aboutPageLabel", { locale: language })}</Text>
                    </View>
                    <View style={styles.answerBox}>
                        <KeyboardAvoidingView
                            keyboardVerticalOffset={-100}
                            behavior={'padding'}
                            style={[formStyle, styles.bottom]}
                        >
                            <View style={styles.formgroup}>
                                <Text style={styles.label}>{I18n.t('namelabel', { locale: language })}</Text>
                                <CustomTextInput
                                    name={'name'}
                                    ref={(ref) => this.nameInputRef = ref}
                                    value={this.state.fullname}
                                    // placeholder={I18n.t('namelabel')}
                                    keyboardType={'default'}
                                    editable={!isLoading}
                                    returnKeyType={'next'}
                                    blurOnSubmit={false}
                                    withRef={true}
                                    onSubmitEditing={() => this.bioInputRef.focus()}
                                    onChangeText={(value) => this.setState({ fullname: value })}
                                    isEnabled={!isLoading}
                                />
                            </View>
                            <View style={styles.formgroup}>
                                <Text style={styles.label}>{I18n.t('dob_label', { locale: language })}</Text>
                                <TouchableOpacity onPress={this._showDateTimePicker}
                                    style={{
                                        height: 52,
                                        borderRadius: 5,
                                        marginTop: 8,
                                        backgroundColor: "rgba(245,245,245,100)", flexDirection: "row", alignItems: "center"
                                    }}>
                                    <Text style={{
                                        paddingLeft: IS_ANDROID ? 5 : 5,
                                        width: metrics.DEVICE_WIDTH * 0.73,
                                        fontFamily: "Avenir-Medium",
                                        fontSize: 17,
                                        color: "#909096"
                                    }}>{getdob}</Text>

                                    <Image style={{ width: 19, height: 21 }}
                                        source={require('../../images/icons/calender_button.png')}
                                        resizeMethod="resize"
                                        resizeMode="contain" />
                                </TouchableOpacity>
                            </View>
                            <View style={styles.formgroup}>
                                <Text style={styles.label}>{I18n.t('gender_label', { locale: language })}</Text>
                                <View style={{
                                    height: 42,
                                    borderWidth: 1,
                                    borderColor: "#F5F5F5",
                                    borderRadius: 21,
                                    flexDirection: "row",
                                    marginTop: 8,
                                    // justifyContent: "space-between",
                                    // alignItems: "center"
                                }}>
                                    {
                                        gender_arr.map((value, key) => {
                                            return (
                                                selectedgender === key ?
                                                    <LinearGradient key={key}
                                                        colors={['#DB3D88', '#273174']}
                                                        start={{ x: 0, y: 1 }}
                                                        end={{ x: 1, y: 1 }}
                                                        style={{ flex: 1, alignItems: "center", justifyContent: "center", backgroundColor: "red", borderRadius: 21 }}
                                                    >

                                                        <Text style={{
                                                            fontFamily: "Avenir-Heavy",
                                                            fontSize: 17,
                                                            color: "#FFF"
                                                        }}>{value}</Text>
                                                    </LinearGradient>
                                                    :
                                                    <TouchableOpacity key={key} onPress={() => this.setState({ selectedgender: key })}
                                                        style={{ flex: 1, alignItems: "center", justifyContent: "center", backgroundColor: "#FFF", borderRadius: 21 }}
                                                    >
                                                        <Text style={{
                                                            fontFamily: "Avenir-Heavy",
                                                            fontSize: 17,
                                                            color: "#909096"
                                                        }}>{value}</Text>
                                                    </TouchableOpacity>
                                            )
                                        })
                                    }
                                </View>
                            </View>
                            <View style={styles.formgroup}>
                                <Text style={styles.label}>{I18n.t('interest_label', { locale: language })}</Text>
                                <View style={{
                                    height: 42,
                                    borderWidth: 1,
                                    borderColor: "#F5F5F5",
                                    borderRadius: 21,
                                    flexDirection: "row",
                                    marginTop: 8,
                                    // justifyContent: "space-between",
                                    // alignItems: "center"
                                }}>
                                    {
                                        interest_arr.map((value, key) => {
                                            return (
                                                interest_gender === key ?
                                                    <LinearGradient key={key}
                                                        colors={['#DB3D88', '#273174']}
                                                        start={{ x: 0, y: 1 }}
                                                        end={{ x: 1, y: 1 }}
                                                        style={{ flex: 1, alignItems: "center", justifyContent: "center", backgroundColor: "red", borderRadius: 21 }}
                                                    >
                                                        <Text style={{
                                                            fontFamily: "Avenir-Heavy",
                                                            fontSize: 17,
                                                            color: "#FFF"
                                                        }}>{value}</Text>
                                                    </LinearGradient>
                                                    :
                                                    <TouchableOpacity key={key} onPress={() => this.setState({ interest_gender: key })}
                                                        style={{ flex: 1, alignItems: "center", justifyContent: "center", backgroundColor: "#FFF", borderRadius: 21 }}>
                                                        <Text style={{
                                                            fontFamily: "Avenir-Heavy",
                                                            fontSize: 17,
                                                            color: "#909096"
                                                        }}>{value}</Text>
                                                    </TouchableOpacity>
                                            )
                                        })
                                    }
                                </View>
                            </View>
                            <View style={styles.formgroup}>
                                <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                                    <Text style={styles.label}>{I18n.t('bio_label', { locale: language })}</Text>
                                    <Text style={styles.label}>{'Min 50 Character'}</Text>

                                </View>
                                <View style={{
                                    // flex: 1,
                                    backgroundColor: 'rgba(245,245,245, 100)',
                                    borderRadius: 5,
                                    margin: IS_ANDROID ? -1 : 0,
                                    height: 140,
                                    // padding: 7,
                                    marginTop: 8,
                                    alignItems: 'flex-start'
                                }}>
                                    <CustomTextInput
                                        style={{
                                            // flex: 1,
                                            width: metrics.DEVICE_WIDTH - 60,
                                            minHeight: 140,
                                            maxHeight: 140,
                                            textAlignVertical: "top",
                                            // backgroundColor:"#000"
                                        }}
                                        placeholder={I18n.t('aboutPageLabel', { locale: language })}
                                        numberOfLines={4}
                                        multiline={true}
                                        name={'bio'}
                                        value={this.state.bio}
                                        ref={(ref) => this.bioInputRef = ref}
                                        keyboardType={'default'}
                                        editable={!isLoading}
                                        returnKeyType={'next'}
                                        blurOnSubmit={true}
                                        withRef={true}
                                        // onSubmitEditing={() => this.phonenumberInputRef.focus()}
                                        onChangeText={(value) => this.setState({ bio: value })}
                                        isEnabled={!isLoading}
                                    />
                                </View>
                            </View>
                        </KeyboardAvoidingView>
                        <View style={[styles.formgroup]}>
                            <Text style={styles.label}>{I18n.t('gallary_label', { locale: language })}</Text>
                            <Gallary
                                token={this.props.token}
                                activateLoading={() => this.activateLoader()}
                                gallary_Images={this.state.gallary_Images}
                                refreshpage={() => this.getUserprofile()}
                                fullname={fullname}
                                dob={dob}
                                selectedgender={selectedgender}
                                interest_gender={interest_gender}
                                bio={bio}
                                gallary_Images={gallary_Images}
                                gallary_Images_update={this.gallary_Images_update.bind(this)}
                            // updateUploadingProgress={this.updateUploadingProgress.bind(this)}
                            />
                        </View>
                        <TouchableOpacity style={[styles.inviteFriend]} onPress={() => this.Addreview()}>
                            <Text style={styles.invitefirend_button_text}>{I18n.t('addreviewtitle', { locale: language })}</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={styles.inviteFriend} onPress={() => this.InviteFriend()}>
                            <Text style={styles.invitefirend_button_text}>{I18n.t('invitefriendtitle', { locale: language })}</Text>
                        </TouchableOpacity>
                        <LinearGradient
                            colors={['#DB3D88', '#273174']}
                            start={{ x: 0, y: 1 }}
                            end={{ x: 1, y: 1 }}
                            style={styles.signInButton}>
                            <CustomButton
                                text={I18n.t('start_button', { locale: language })}
                                // onPress={() => navigate('questionnaire')}
                                onPress={() => this.updateProfile()}
                                textStyle={styles.signInButtonText}
                            />
                        </LinearGradient>
                    </View>
                </ScrollView>
                <DateTimePicker
                    isVisible={isDateTimePickerVisible}
                    onConfirm={this._handleDatePicked}
                    onCancel={this._hideDateTimePicker}
                />
                {/* share  */}
                <ShareSheet visible={this.state.review_visible} onCancel={() => this.onCancel()}>
                    <Button iconSrc={{ uri: EMAIL_ICON }}
                        onPress={() => {
                            this.onCancel();
                            setTimeout(() => {
                                Share.shareSingle(Object.assign(reviewOptions, {
                                    "social": "email"
                                }));
                            }, 300);
                        }}>Email</Button>
                </ShareSheet>
                <ShareSheet visible={this.state.friends_invite_visible} onCancel={() => this.onCancel()}>
                    <Button iconSrc={{ uri: TWITTER_ICON }}
                        onPress={() => {
                            this.onCancel();
                            setTimeout(() => {
                                Share.shareSingle(Object.assign(shareOptions, {
                                    "social": "twitter"
                                }));
                            }, 300);
                        }}>Twitter
                </Button>
                    <Button iconSrc={{ uri: FACEBOOK_ICON }}
                        onPress={() => {
                            this.onCancel();
                            setTimeout(() => {
                                Share.shareSingle(Object.assign(shareOptions, {
                                    "social": "facebook"
                                }));
                            }, 300);
                        }}>Facebook</Button>
                    <Button iconSrc={{ uri: WHATSAPP_ICON }}
                        onPress={() => {
                            this.onCancel();
                            setTimeout(() => {
                                Share.shareSingle(Object.assign(shareOptions, {
                                    "social": "whatsapp"
                                }));
                            }, 300);
                        }}>Whatsapp</Button>
                    <Button iconSrc={{ uri: GOOGLE_PLUS_ICON }}
                        onPress={() => {
                            this.onCancel();
                            setTimeout(() => {
                                Share.shareSingle(Object.assign(shareOptions, {
                                    "social": "googleplus"
                                }));
                            }, 300);
                        }}>Google +</Button>

                    <Button iconSrc={{ uri: PINTEREST_ICON }}
                        onPress={() => {
                            this.onCancel();
                            setTimeout(() => {
                                Share.shareSingle(Object.assign(shareOptions, {
                                    "social": "pinterest"
                                }));
                            }, 300);
                        }}>Pinterest</Button>
                    <Button
                        iconSrc={{ uri: CLIPBOARD_ICON }}
                        onPress={() => {
                            this.onCancel();
                            setTimeout(() => {
                                if (typeof shareOptions["url"] !== undefined) {
                                    Clipboard.setString(shareOptions["url"]);
                                    if (Platform.OS === "android") {
                                        ToastAndroid.show('Link copiado al portapapeles', ToastAndroid.SHORT);
                                    } else if (Platform.OS === "ios") {
                                        AlertIOS.alert('Link copiado al portapapeles');
                                    }
                                }
                            }, 300);
                        }}>Copy Link</Button>
                    <Button iconSrc={{ uri: MORE_ICON }}
                        onPress={() => {
                            this.onCancel();
                            setTimeout(() => {
                                Share.open(shareOptions)
                            }, 300);
                        }}>More</Button>
                </ShareSheet>
                <Modal
                    isVisible={is_uploadFile}
                    scrollTo={this.handleScrollTo}
                    style={{ margin: 0, justifyContent: "center", alignItems: "center", flexDirection: "column" }}>
                    <Text style={{ color: "#fff", fontSize: 15 }}> {this.state.loadingProgress} %</Text>
                    <MediafileLoader />
                </Modal>
                {/* share close */}
            </SafeAreaView>
        )
    }
}
class Gallary extends BaseFormComponent {
    constructor(props) {
        super(props);
        this.state = {
            fullname: props.fullname,
            dob: props.dob,
            gallary_Images: [],
            image_0: { "uri": '', "order": 0 },
            image_1: { "uri": '', "order": 0 },
            image_2: { "uri": '', "order": 0 },
            image_3: { "uri": '', "order": 0 },
            image_4: { "uri": '', "order": 0 },
            image_5: { "uri": '', "order": 0 },
            source_url: null,
            thumbnail_image: null,
            thumblinefiletype: '',
            imageSelect: false,
            videoSelect: false,
            thumblinename: '',
            order_number: 1
        };
    }
    showActionSheet = (order) => {
        this.setState({
            order_number: order
        }, () => this.ActionSheet.show())
    }

    componentDidMount() {
        var i = 0;
        while (i < 6) {
            var key = "image_" + i
            var val = {
                "uri": '',//"http://10.2.2.43:4069/public/upload/Photo_11547895346356.png",
                "order": i + 1
            }
            var obj = {}
            obj[key] = val
            console.log("all stated data", obj)
            this.setState(obj)
            i++;
        }
    }
    componentWillReceiveProps = (nextProps, nextState) => {
        let data = nextProps.gallary_Images;
        console.log("componentWillReceiveProps", data)
        this.setState({
            fullname: nextProps.fullname,
            dob: nextProps.dob,
            gallary_Images: nextProps.gallary_Images
        })

        var data_arr = [];
        for (var i = 0; i < data.length; i++) {
            var arr = {};
            arr.order = data[i].order;
            arr._id = data[i]._id;
            if (data[i].isVideo) {
                arr.uri = data[i].thumbImage;
                arr.isVideo = data[i].isVideo;
            } else {
                arr.uri = data[i].uri ? data[i].uri : data[i].url;
            }
            data_arr.push(arr)
        }
        var i = 0;
        while (i < data_arr.length) {
            var key = "image_" + i
            var val = data_arr[i]
            var obj = {}
            obj[key] = val
            this.setState(obj)
            i++;
        }
    }
    selectFile = (index) => {
        if (index === 0) {
            this.selectPhotoTapped()
        } else if (index === 1) {
            this.selectVideoTapped()
        }
    }
    selectPhotoTapped() {
        const { fullname, dob, gallary_Images, order_number } = this.state;

        const options = {
            quality: 0.6,
            maxWidth: 500,
            maxHeight: 500,
            storageOptions: {
                cameraRoll: true,
                waitUntilSaved: true,
            }
        };
        ImagePicker.showImagePicker(options, (response) => {
            console.log('Response = ', response);
            if (response.didCancel) {
                console.log('User cancelled photo picker');
            }
            else if (response.error) {
                console.log('ImagePicker Error: ', response.error);
            }
            else if (response.customButton) {
                console.log('User tapped custom button: ', response.customButton);
            }
            else {
                let setLocal = {};
                let source = { uri: response.uri };
                let name = response.fileName;
                let url = response.uri;
                let path = (Platform.OS === 'ios') ? url.replace(/^file:\/\//, '') : response.uri
                console.log("selectPhotoTapped Path:=", response)
                let filetype = 'image/jpg';
                let is_video = false;

                setLocal.uri = path;
                setLocal.filetype = filetype;
                setLocal.is_video = is_video;
                setLocal.order = order_number.toString();
                setLocal.is_Update = true;
                setLocal.filename = name;

                this.setState({
                    // source_url: response.uri,
                    thumbnail_image: source,
                    thumblinefiletype: response.type,
                    imageSelect: true,
                    videoSelect: false,
                    thumblinename: name,
                    // });
                }, this.props.gallary_Images_update(setLocal))
                // () => this.upload_pic(path, filetype, name, is_video))
            }
        });
    }
    selectVideoTapped() {
        const { fullname, dob, gallary_Images, order_number } = this.state;
        const options = {
            title: 'Video Picker',
            takePhotoButtonTitle: 'Take Video...',
            mediaType: 'video',
            videoQuality: 'medium',
        };
        let is_video = true;
        let filetype = "video";
        let is_update = true;
        ImagePicker.showImagePicker(options, (response) => {
            console.log('Response = ', response);
            if (response.didCancel) {
                console.log('User cancelled video picker');
            } else if (response.error) {
                console.log('ImagePicker Error: ', response.error);
            } else if (response.customButton) {
                console.log('User tapped custom button: ', response.customButton);
            } else {
                let setLocal = {};
                let filename = response.path;
                let url = response.uri;
                let path = (Platform.OS === 'ios') ? url.replace(/^file:\/\//, '') : response.uri

                setLocal.uri = path;
                setLocal.filetype = filetype;
                setLocal.is_video = is_video;
                setLocal.order = order_number.toString();
                setLocal.is_Update = is_update;
                setLocal.filename = filename;

                this.setState({
                    videoSource: response.uri,
                }, this.props.gallary_Images_update(setLocal))

                // }, () => this.upload_pic(path, filetype, filename, is_video, is_update));
            }
        });
    }
    // upload_pic = (filelink, filetype, filename, is_video) => {
    //     // const { thumblinefiletype, thumblinename } = this.state;
    //     this.props.activateLoading()
    //     let header = {
    //         'Authorization': this.props.token,
    //     };
    //     // const details = new FormData();
    //     // details.append('file', {
    //     //     uri: filelink,
    //     //     type: filetype,
    //     //     name: filename
    //     // });
    //     var details = {
    //         url: filelink,
    //         type: filetype,
    //         name: filename
    //     };
    //     let Is_update = true
    //     console.log("uploadFile", details)
    //     ApiManager.callRnFetchblobFileUploader('POST', 'api/uploadFile', header, details, (progress) => {
    //         this.props.updateUploadingProgress(progress)
    //     }, (success) => {
    //         var response = JSON.parse(success.data);
    //         console.log("fileUpload", response)
    //         this.props.activateLoading()

    //         if (response.status === 0) {
    //             this.showSimpleMessage("danger", { backgroundColor: "#DC6666" }, response.message, '')
    //             return
    //         }
    //         else if (response.status === 1) {
    //             this.updateUserProfile(response.data.filepath, is_video, Is_update)
    //             // this.props.refreshpage
    //         }
    //     }, (error) => {
    //         console.log("error", error)
    //     })
    // }
    updateUserProfile = (filepath, is_video, is_update) => {
        console.log("success filepath", filepath)
        const { fullname, dob, gallary_Images, order_number } = this.state;
        const { bio, selectedgender, interest_gender, } = this.props;

        var arr2 = [];
        let aaar = {}
        aaar.order = order_number.toString();
        aaar.url = filepath;
        arr2.push(aaar);
        let new_array = [];
        gallary_Images.map(obj => arr2.find(o => o.order === obj.order) || new_array.push(obj));

        let newimage = {};
        newimage.url = filepath;
        newimage.order = order_number.toString();
        newimage.isVideo = is_video;
        newimage.isUpdated = is_update
        // new_array.push(newimage)

        let header = {
            'Authorization': this.props.token,
            'Content-Type': 'application/json'
        };
        const details = {
            // "fullName": fullname,
            // "gender": gender_arr[selectedgender].toLowerCase(),
            // "genderIntrestedIn": gender_arr[interest_gender].toLowerCase(),
            // "dob": dob,
            // "bio": bio,
            "gallery": [...new_array, newimage]
        }
        console.log("details", details)
        // ApiManager.callwebservice('POST', 'api/updateUserProfile', header, details, (success) => {
        ApiManager.callwebservice('POST', 'api/editUserGallery', header, details, (success) => {
            let response = JSON.parse(success._bodyInit);
            console.log()
            if (response.status === 0) {
                this.showSimpleMessage("danger", { backgroundColor: "#DC6666" }, response.message, '')
                console.log("error upload pics", response)
                return
            }
            else if (response.status === 1) {
                console.log("success upload pics", response)
                this.props.refreshpage()
            }
        }, (error) => {
            console.log("error upload pics", error)
        })

    }
    VideoDraphics() {
        return (
            <View style={{ position: "absolute", zIndex: 3, height: 17, width: 17, top: 7, left: 7, borderRadius: 8 }}>
                <Image source={require('../../images/icons/Videos.png')} style={{ height: '100%', width: '100%' }} />
            </View>
        )
    }
    render() {
        console.log("images are", this.state);
        return (
            <View style={[styles.elevated_shdow, { flex: 1, marginTop: 8 }]}>
                <View style={{ flexDirection: "row" }}>
                    <TouchableOpacity style={[styles.elevated_shdow, { flex: 6.5, borderRadius: 10, overflow: "hidden" }]} onPress={() => this.showActionSheet(this.state.image_0.order)} >
                        {
                            this.state.image_0.isVideo ? this.VideoDraphics() : undefined
                        }
                        <Image
                            source={!this.state.image_0.uri ? require('../../images/applogo.png') : this.state.image_0}
                            // source={require('../../images/Photo_1.png')}
                            style={{
                                flex: 1,
                                width: null,
                                height: null,
                            }}
                            resizeMethod="resize"
                        // resizeMode="contain"
                        />
                        <Text style={[styles.elevated_shdow, styles.image_badge]}>{this.state.image_0.order}</Text>
                    </TouchableOpacity>
                    <View style={{ justifyContent: "space-between", alignItems: 'flex-end', flex: 3.5, }}>
                        <TouchableOpacity style={[styles.elevated_shdow, {
                            borderRadius: 5,
                            // marginVertical: 5,
                            width: metrics.DEVICE_WIDTH * 0.25,
                            height: metrics.DEVICE_WIDTH * 0.25,
                            overflow: "hidden"
                        }]}
                            onPress={() => this.showActionSheet(this.state.image_1.order)}>
                            {this.state.image_1.isVideo ? this.VideoDraphics() : undefined}
                            <Image
                                source={!this.state.image_1.uri ? require('../../images/applogo.png') : this.state.image_1}
                                style={{
                                    flex: 1,
                                    width: null,
                                    height: null,
                                }}
                                resizeMethod="resize"
                            />
                            <Text style={[styles.elevated_shdow, styles.image_badge]}>{this.state.image_1.order}</Text>
                        </TouchableOpacity>
                        <View style={{ height: 5 }} />
                        <TouchableOpacity
                            style={[styles.elevated_shdow, {
                                borderRadius: 5,
                                // marginVertical: 5,
                                width: metrics.DEVICE_WIDTH * 0.25,
                                height: metrics.DEVICE_WIDTH * 0.25,
                                backgroundColor: "#696969",
                                overflow: "hidden"
                            }]}
                            onPress={() => this.showActionSheet(this.state.image_2.order)}>
                            {this.state.image_2.isVideo ? this.VideoDraphics() : undefined}
                            <Image
                                source={!this.state.image_2.uri ? require('../../images/applogo.png') : this.state.image_2}
                                style={{
                                    flex: 1,
                                    width: null,
                                    height: null,
                                }}
                                resizeMethod="resize"
                                resizeMode="contain"
                            />
                            <Text style={[styles.elevated_shdow, styles.image_badge]}>{this.state.image_2.order}</Text>
                        </TouchableOpacity>
                    </View>
                </View>
                <View style={{ height: metrics.DEVICE_WIDTH * 0.28, flexDirection: "row", justifyContent: "space-between", alignItems: 'flex-end' }}>
                    <TouchableOpacity style={[styles.elevated_shdow, {
                        borderRadius: 5, width: metrics.DEVICE_WIDTH * 0.25,
                        height: metrics.DEVICE_WIDTH * 0.25,
                        backgroundColor: "transparent",
                        overflow: "hidden"
                    }]}
                        onPress={() => this.showActionSheet(this.state.image_3.order)}>
                        {this.state.image_3.isVideo ? this.VideoDraphics() : undefined}
                        <Image
                            source={!this.state.image_3.uri ? require('../../images/applogo.png') : this.state.image_3}
                            style={{
                                flex: 1,
                                width: null,
                                height: null,
                            }}
                            resizeMethod="resize"
                        // resizeMode="contain"
                        />
                        <Text style={[styles.elevated_shdow, styles.image_badge]}>{this.state.image_3.order}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.elevated_shdow, {
                        borderRadius: 5, width: metrics.DEVICE_WIDTH * 0.25,
                        height: metrics.DEVICE_WIDTH * 0.25,
                        backgroundColor: "transparent",
                        overflow: "hidden"
                    }]} onPress={() => this.showActionSheet(this.state.image_4.order)}>
                        {this.state.image_4.isVideo ? this.VideoDraphics() : undefined}
                        <Image
                            source={!this.state.image_4.uri ? require('../../images/applogo.png') : this.state.image_4}
                            style={{
                                flex: 1,
                                width: null,
                                height: null,
                            }}
                            resizeMethod="resize"
                        // resizeMode="contain"
                        />
                        <Text style={[styles.elevated_shdow, styles.image_badge]}>{this.state.image_4.order}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.elevated_shdow, {
                        borderRadius: 5, width: metrics.DEVICE_WIDTH * 0.25,
                        height: metrics.DEVICE_WIDTH * 0.25,
                        backgroundColor: "transparent",
                        overflow: "hidden"
                    }]}
                        onPress={() => this.showActionSheet(this.state.image_5.order)}>
                        {this.state.image_5.isVideo ? this.VideoDraphics() : undefined}

                        <Image
                            source={!this.state.image_5.uri ? require('../../images/applogo.png') : this.state.image_5}
                            style={{
                                flex: 1,
                                width: null,
                                height: null,
                            }}
                            resizeMethod="resize"
                        />
                        <Text style={[styles.elevated_shdow, styles.image_badge]}>{this.state.image_5.order}</Text>
                    </TouchableOpacity>
                </View>
                <ActionSheet
                    ref={o => this.ActionSheet = o}
                    title={'Which File Do You like to Upload ? '}
                    options={['Image', 'Video', 'Cancel']}
                    cancelButtonIndex={2}
                    destructiveButtonIndex={1}
                    onPress={(index) => { this.selectFile(index) }}
                />
            </View>
        )
    }
}
const styles = StyleSheet.create({
    container: {
        marginVertical: 20
    },
    grid: {
        flexDirection: "row",
    },
    questionBox: {
        paddingHorizontal: 30
    },
    questionTitle: {
        color: '#313138',
        fontSize: 30,
        marginTop: 10,
        fontFamily: 'Avenir-Heavy',
    },
    label: {
        paddingHorizontal: IS_ANDROID ? 3 : 0,
        color: "#D43C87",
        fontFamily: "Avenir-Medium",
        fontSize: 11
    },
    answerBox: {
        paddingHorizontal: 32,
    },
    formgroup: {
        paddingVertical: 9
    },
    signInButton: {
        borderRadius: 5,
    },
    signInButtonText: {
        fontFamily: "Avenir-Heavy",
        fontSize: 17,
        color: "#fff"
    },
    inviteFriend: {
        marginVertical: 5,
        height: 42,
        justifyContent: "center",
        alignItems: "center",
        borderWidth: 1,
        borderColor: "#F5F5F5",
        borderRadius: 5
    },
    image_badge: {
        backgroundColor: "#FFF",
        position: "absolute",
        zIndex: 1,
        width: 20,
        height: 20,
        borderRadius: 10,
        textAlign: "center",
        bottom: 11,
        right: 5,
        overflow: "hidden"
    },
    invitefirend_button_text: {
        fontFamily: "Avenir-Heavy",
        fontSize: 17,
        color: "#273174"
    },
    elevated_shdow: {
        shadowOffset: { width: 0, height: 2 },
        shadowOpacity: 0.5,
        shadowRadius: 2,
        elevation: 3,
        // shadowOffset: { width: 10, height: 10 },
        shadowColor: 'black',
        // shadowOpacity: 1,
        // elevation: 3,
        // backgroundColor : "#FFF" // invisible color

    }
});
mapStateToProps = (state) => {
    return {
        language: state.language.defaultlanguage,
        data: state.auth.data,
        token: state.auth.token
    }
}
mapDispatchToProps = (dispatch) => {
    return bindActionCreators({
        saveprofileStatus: actions.visitprofile,
        saveNumber: queactions.savegivenanswerNumber
    }, dispatch);
}
export default connect(mapStateToProps, mapDispatchToProps)(Basicinfo);


//  twitter icon
const TWITTER_ICON = "data:image/png;base64,";

//  facebook icon
const FACEBOOK_ICON = "data:image/png;base64,";

//  whatsapp icon
const WHATSAPP_ICON = "data:image/png;base64,";

//  gplus icon
const GOOGLE_PLUS_ICON = "data:image/png;base64,";

//  email icon
const EMAIL_ICON = "data:image/png;base64,";

//  pinterest icon
const PINTEREST_ICON = "data:image/png;base64,"

//  clipboard icon
const CLIPBOARD_ICON = "data:image/png;base64,";

//  more icon
const MORE_ICON = "data:image/png;base64,";


const REACT_ICON = 'data:image/png;base64,';

...
class Sidebar extends BaseFormComponent {
    constructor(props) {
        super(props);
        this.state = {
    ...
    upload_pic = (filelink, filetype, filename, is_video) => {
        ...
        ApiManager.callRnFetchblobFileUploader('POST', 'api/uploadFile', header, details, (progress) => {
            ...
            if (response.status === 0) {
                ...
                this.updateUserProfilePic(response.data.filepath, is_video, Is_update)
    ...
    updateUserProfilePic= async (filepath)=>{
    ...
    logoutApplication = async () => {
        ...
        await ApiManager.callwebservice('POST', 'api/logout', header, {}, (success) => {
            let response = JSON.parse(success._bodyInit);
    ...
    render() {
        ...
        const currentPage = this.props.navigation.state.routes[this.props.navigation.state.index].routeName;
        return (
            <>
            <LinearGradient colors={['#DB3D88', '#273174']} style={{ flex: 1 }}>
                <SafeAreaView style={{ flex: 1 }}>
                    <StatusBar barStyle="light-content" backgroundColor={IS_ANDROID ? "#DB3D88" : undefined} />
                    <TouchableOpacity
                        onPress={() => this.props.navigation.closeDrawer()}
                        style={{ height: IS_ANDROID ? 50 : 64, alignSelf: "flex-start", justifyContent: "flex-end", position: "relative", zIndex: 1,  }}>
                        <Close />
                    </TouchableOpacity>
                    {
                        ...
                        <View style={styles.profile}>
                            <TouchableOpacity style={{ overflow: "hidden" ,width: 100, height: 100, alignItems: "center",  justifyContent: "space-around" }} onPress={()=>this.selectPhotoTapped()}>
                                <Image
                                    ...
                                    resizeMethod="resize"
                                    />
                                <View style={{ height:20, width:20, backgroundColor:"#FFF", position:"absolute", zIndex:5, bottom:10, right:0,  borderRadius:10, borderWidth:1 , borderColor:"#F5F5F5"}}/>
                            </TouchableOpacity>
                            <Text style={styles.usertitle}>{data ? data.fullName : "guest"}</Text>
                            ...
                            <ScrollView  style={styles.menu}>
                                {
                                    menu_array.map((item, index) => {
                                        ...
                                        return <TouchableOpacity key={index} onPress={() => navigate(item.action_Page)} 
                                        style={{ justifyContent:"center", alignItems:"center" }} >
                                            <Text style={[styles.label, {color:item.action_Page===currentPage ? "#000": "rgba(255, 255, 255, 0.7)" }]}>{I18n.t(item.label)}</Text>
                                        </TouchableOpacity>
                                    })
                                }
                            <TouchableOpacity style={styles.logout} onPress={() => this.InviteFriend()}>
                                <Text style={[styles.label, {color: "rgba(255, 255, 255, 0.7)" }]}>{I18n.t('invitefriendtitle', { locale: language })}</Text>
                            </TouchableOpacity>

                            <TouchableOpacity style={styles.logout} onPress={() => this.logoutApplication()}>
                            <Text style={[styles.label, { color: "rgba(255, 255, 255, 0.7)", }]}>{I18n.t('logout_label')}</Text>
                        </TouchableOpacity>
                        </ScrollView>
